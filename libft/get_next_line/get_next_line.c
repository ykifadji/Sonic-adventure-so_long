...
/*   Updated: 2023/03/04 16:26:53 by ykifadji         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*trim_temp(char *temp, char *final)
{
	...
	while (temp[i] != '\n')
		i++;
	before_nl = ft_substrgnl(temp, 0, i + 1);
	after_nl = ft_substrgnl(temp, i + 1, (ft_strlen(temp) - i));
	final = ft_strjoingnl(final, before_nl, 1);
...
char	*trim_buf(char *temp, char *final, char *buf)
{
	...
	while (buf[i] != '\n')
		i++;
	before_nl = ft_substrgnl(buf, 0, i + 1);
	after_nl = ft_substrgnl(buf, i + 1, ft_strlen(buf) - i);
	final = ft_strjoingnl(final, before_nl, 1);
	ft_strcpy(temp, after_nl);
	free(after_nl);
	return (final);
...
char	*get_l(char *temp, char *final, char *buf, int ret)
{
	...
	if (ft_strchr(buf, '\n'))
		...
		final = ft_strjoingnl(final, buf, 0);
	return (final);
}
...
char	*get_next_line(int fd)
{
	...
	var.final = ft_strjoingnl(var.final, temp[fd], 0);
